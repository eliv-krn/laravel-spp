          ...
          <div class="card card-primary">
            ...
            <form action="{{ route('pembayaran.update', $pembayaran->id_pembayaran) }}" method="POST">
              ...
              <div class="card-body">
                ...
                <div class="form-group">
                  ...
                  <select name="bulan_dibayar" class="form-control @error('bulan_dibayar') is-invalid @enderror" id="bulan_dibayar">
                    ...
                    @foreach ($bulan as $bln)
                        <option value="{{ old('bulan_dibayar', $bln) }}" {{ ($bln == $pembayaran->bulan_dibayar) ? ' selected' : '' }}>{{ $bln }}</option>
                    @endforeach
                  </select>
                  @error('bulan_dibayar')
                      <div class="invalid-feedback">
                ...
                <div class="form-group">
                  ...
                  <select name="tahun_dibayar" class="form-control @error('tahun_dibayar') is-invalid @enderror" id="tahun_dibayar">
                    <option value="">-- Pilih --</option>
                    @foreach ($tahun as $th)
                        <option value="{{ old('tahun_dibayar', $th) }}" {{ ($th == $pembayaran->tahun_dibayar) ? ' selected' : '' }}>{{ $th }}</option>
                    @endforeach
                  </select>
                  @error('tahun_dibayar')
                      <div class="invalid-feedback">
                ...
                <div class="form-group">
                  ...
                  <select name="id_spp" class="form-control @error('id_spp') is-invalid @enderror" id="id_spp">
                    <option value="">-- Pilih --</option>
                    @foreach ($spp as $dt)
                        <option value="{{ old('id_spp', $dt->id_spp) }}" {{ ($dt->id_spp == $pembayaran->id_spp) ? ' selected' : '' }}>{{ $dt->tahun }}</option>
                    @endforeach
                  </select>
                  @error('id_spp')
                      <div class="invalid-feedback">
                ...
                <div class="form-group">
                  ...
                  <input type="number" name="jumlah_bayar" class="form-control @error('jumlah_bayar') is-invalid @enderror" id="jumlah_bayar" placeholder="Masukkan jumlah bayar" value="{{ old('jumlah_bayar', $pembayaran->jumlah_bayar) }}">
                  @error('jumlah_bayar')
                      <div class="invalid-feedback">
                          {{ $message }}
                      </div>
              ...
              <div class="card-footer">
                ...
                <a href="{{ route('pembayaran.index') }}" class="btn btn-danger">Kembali</a>
              </div>
            </form>
          </div>
          <!-- /.card -->
